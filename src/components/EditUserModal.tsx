...
import { updateUserAction } from "@/app/dashboard/actions/user.action";
...
export default function EditUserModal({ r, open, onClose }: Props) {
    const [loading, setLoading] = useState(false);
    const [error, setError] = useState("");
    const [pid, setPid] = useState(r.pid);
    const [name, setName] = useState(r.name);
    const [email, setEmail] = useState(r.email);
    const [isActive, setIsActive] = useState<boolean>(r.isActive);
    const [plateNumber, setPlateNumber] = useState<string[]>(r.plateNumber?.length ? r.plateNumber : [""]);
    ...
    useEffect(() => {
        if (open) {
            ...
            setPlateNumber(r.plateNumber?.length ? r.plateNumber : [""]);
            setError("");
        }
    }, [open]);
    ...
    const handleSubmit = async () => {
        ...
        const result = await updateUserAction(r.id, {
            pid: pid.trim(),
            name: name.trim(),
            email: email.trim(),
            isActive,
    ...
    return (
        <Dialog open={open} onOpenChange={(val) => { if (!val) onClose(); }}>
            <DialogContent className="bg-background/95 backdrop-blur-xl border-border/40 shadow-2xl max-w-md max-h-[85vh] overflow-y-auto">
                <DialogHeader>
                    <DialogTitle className="text-sm">Edit Penghuni</DialogTitle>
                    <DialogDescription className="text-[10px]">ID: {r.id}</DialogDescription>
                </DialogHeader>
